...
$\mathcal{O}(N^2)$ operation (instead of $\mathcal{O}(N^3)$ for a general matrix inverse).\footnote{
    ...
    U^{-1}$, the linear equation $\b U\b x = \b y$ is solved each time the matrix inverse is needed.
}
...
\begin{algorithmic}[1]
    ...
    \Function{CLEAN}{$\b a$}
    \Let{$\b M$}{$\b B^*\b B^*$}
    \Let{$\b U$}{${\rm chol}(\b B)$} \Comment{Cholesky decomposition}
...
\begin{table*}[t]
    ...
    \begin{tabular}{cccccccccc}
        ...
        \hline
        1 & 36.528 & 0.024 & 16.2 & 372.8 & & & no & yes \\
